...
pub struct CooldownTracker {
    ...
    channel_invocations: HashMap<serenity::GenericChannelId, (Instant, u64)>,
...
impl CooldownTracker {
    ...
    pub fn start_cooldown(&mut self, ctx: CooldownContext) {
        ...
        self.user_invocations
            .insert(ctx.user_id, (now, STARTING_BURST_AMOUNT));
        self.channel_invocations
            .insert(ctx.channel_id, (now, STARTING_BURST_AMOUNT));

        if let Some(guild_id) = ctx.guild_id {
            self.guild_invocations
                .insert(guild_id, (now, STARTING_BURST_AMOUNT));
            self.member_invocations
